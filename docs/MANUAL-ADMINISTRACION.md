...
## 17. Panel Admin (avisos, eventos, estado local, comercios, papelera, dashboard)
...
"Eliminar" en Avisos, Eventos, Comercios y Radar Social ya no borra directamente: mueve un snapshot completo del documento a `admin_papelera` (con la colección de origen y el id original) y borra el original. Desde la pestaña Papelera se puede Restaurar (recrea el doc con el mismo id — importante en Eventos, cuyo id determina el slug de `/planes/`) o Eliminar definitivamente. Sin expiración automática todavía (la papelera crece indefinidamente hasta que se purga a mano).

### Dashboard

Pestaña de inicio con contadores en vivo (avisos/eventos/comercios/reportes activos, elementos en papelera), calculados en el cliente a partir de los datos que ya cargan las demás pestañas — sin lecturas extra a Firestore. **No incluye tráfico ni visitas**: no hay ninguna herramienta de analítica instalada en el sitio; añadirla sería una decisión de producto aparte (proveedor, privacidad, coste).

### Estado actual (4 fases del plan original + 4 extras)
...
2. `.github/workflows/sync-admin-panel.yml` corre cada 15 min (+ manual): ejecuta `node scripts/sync-admin-firestore.js`, que lee esas 3 fuentes **más `radar_reports`** con `firebase-admin` y reconstruye `data/avisos-locales.json`, `data/agenda-local.json`, `data/estado-local.json` y `data/radar-trafico.json`:
   ...
   - **Eventos**: reemplaza el subconjunto `fuente:"manual"` (id derivado y estable como `manual-{docId de Firestore}`) **más** cualquier evento importado "reclamado" desde el panel (ver sección 18), dejando intactos el resto de eventos `"ayuntamiento"`, `"alhaurinhoy"` y `"legado"` (3 eventos de la Virgen de Gracia que no tenían `fuente` antes de esto, ver commit de fase 3).
   ...
   - **Tráfico automático** (1er automatismo de Estado Local): si hay un reporte activo del Radar Social de tipo `corte-trafico` o `arroyo` (no expirado por TTL, no descartado por los vecinos), `data/radar-trafico.json` lo recoge, y `mergeRadarTrafico()` (en `js/home-live.js` y `scripts/render-home-widgets-static.js`, mismo patrón que `mergeAvisosOficiales`) sustituye automáticamente la tarjeta "Tráfico" — sin tocar el panel admin. Prioridad: base manual < Radar Social (automático) < aviso local manual publicado expresamente.
   - **Agenda automática** (2º automatismo de Estado Local): la tarjeta "Agenda" se sustituye por un resumen calculado en cada render a partir de `data/agenda-local.json` (`mergeAgendaSummary()`, misma ventana de 3 días que ya usa `js/home-agenda.js`/`renderAgenda` para la lista completa — el filtro se extrajo a `getUpcomingAgendaEvents()` para no duplicarlo). Con eventos próximos casi siempre hay alguno, así que en la práctica esta tarjeta pasa a estar prácticamente siempre auto-generada; el valor manual del panel admin solo se ve cuando no hay ningún evento en esos 3 días. Misma prioridad que Tráfico: base < automático < aviso local manual.
...
## 18. Eventos importados en el panel admin (edición y "reclamado")

Los eventos importados automáticamente de alhaurinhoy.es y del Ayuntamiento (270+ eventos, `fuente:"alhaurinhoy"`/`"ayuntamiento"`) viven solo en `data/agenda-local.json`, escritos directamente por `scripts/actualizar_agenda_alhaurinhoy.py` / `actualizar_agenda_ayto.py` — nunca pasaron por Firestore. Motivado por un bug real: varios eventos de "Cine de Verano" quedaron mal categorizados (Gastronomía, Música en vivo) porque no existía la categoría "Cine" y el texto colisionaba con otras palabras clave, y no había forma de corregirlo sin editar el JSON a mano.

**Pestaña Eventos**: ahora lista los importados junto a los manuales (fetch directo de `data/agenda-local.json`, dato público, combinado con el listener en vivo de `admin_eventos`). El campo Tipo es un `<select>` con el vocabulario real de `guess_category()` (`Cultos y procesiones`, `Deportes`, `Motor`, `Cine`, `Música en vivo`, `Gastronomía`, `Ocio`, `Evento`) en vez de texto libre. Incluye un filtro por título dado el volumen.
...
1. El `account_id` (`GET /accounts`).
2. El `site_tag` de Web Analytics (`GET /accounts/{id}/rum/site_info/list`, buscando el site cuyo `site_token` coincide con el beacon ya instalado — el campo `host` no hacía match exacto con `alhaurinaldia.es` en la práctica, así que se usa `site_token` por ser un identificador único sin ambigüedad).
...
El token necesita **dos** permisos de cuenta (no de zona): `Account Analytics: Read` para consultar la GraphQL API, y `Account Settings: Read` — sin este segundo, `GET /accounts` devuelve la lista vacía y el script no puede ni averiguar el `account_id`.

### Checklist de activación (pasos manuales, fuera del repositorio)

- [x] API Token en Cloudflare con permisos **Account → Account Analytics → Read** y **Account → Account Settings → Read**.
- [x] Guardado como GitHub Secret `CLOUDFLARE_ANALYTICS_TOKEN`.
- [x] Verificado en producción el 18/08/2026: sync disparado a mano, `data/analytics-resumen.json` con datos reales (3 visitas / 11 páginas vistas en 24h en el primer run), confirmado visualmente en el dashboard del panel.
