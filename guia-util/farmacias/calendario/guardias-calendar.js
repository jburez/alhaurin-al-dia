...
function renderToday(){if(!todayBox){return;}var f=farmaciaByDay(todayKey);if(!f){todayBox.innerHTML='<div><span class="section-kicker">Guardia de hoy</span><strong>Guardia pendiente de confirmar</strong><p>No consta una farmacia asignada para hoy en nuestro calendario. Consulta la fuente oficial antes de desplazarte.</p></div><div class="guard-actions"><a href="https://alhaurinelgrande.es/farmacias/" target="_blank" rel="noopener noreferrer">Fuente oficial</a></div>';return;}todayBox.innerHTML='<div><span class="section-kicker">Guardia de hoy</span><strong>'+f.nombre+'</strong><p>'+f.direccion+' · Guardia orientativa de 9:30 a 9:30. Confirma siempre en la fuente oficial.</p></div><div class="guard-actions"><a href="'+f.url+'">Ver ficha</a><a class="secondary" href="tel:'+f.telefonoHref+'">'+f.telefono+'</a></div>';}
function render(){title.textContent=meses[month]+' 2026';select.value=String(month);grid.innerHTML='';var first=new Date(year,month,1);var last=new Date(year,month+1,0);var offset=(first.getDay()+6)%7;for(var i=0;i<offset;i++){var e=document.createElement('div');e.className='calendar-day empty';grid.appendChild(e);}for(var d=1;d<=last.getDate();d++){var date=new Date(year,month,d);var k=key(date);var f=farmaciaByDay(k);var cell=document.createElement('article');cell.className='calendar-day'+(k===todayKey?' today':'');var guard=f?'<a class="guard-link" href="'+f.url+'">'+f.nombre+'</a><span class="guard-phone">'+f.telefono+'</span>':'<span class="missing-chip">Sin dato</span><span class="guard-phone">Consulta la fuente oficial</span>';cell.innerHTML='<div class="day-number">'+d+'</div>'+(k===todayKey?'<span class="today-chip">Hoy</span>':'')+guard;grid.appendChild(cell);}}
