(function () {
  ...
  function renderGrid(year, month) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      events.slice(0, 3).forEach(e => {
        ...
        const slug = slugMap[e.id];
        const href = slug ? `/planes/${slug}/` : "#";
        pills += `<a class="cal-pill" href="${esc(href)}" style="background:${cat.color}22;color:${cat.color};border-left:3px solid ${cat.color}" title="${esc(e.titulo)}">${esc(title)}</a>`;
      });
      if (events.length > 3) {
  ...
  function renderList(year, month) {
    ...
    keys.forEach(key => {
      ...
      events.forEach(e => {
        ...
        html += `<a class="cal-event-card" href="${esc(href)}" style="border-left:4px solid ${cat.color}">
  ...
  Promise.all([
    fetch("../data/agenda-local.json").then(r => { if (!r.ok) throw new Error(); return r.json(); }),
    fetch("../data/evento-slugs.json").then(r => r.ok ? r.json() : {}).catch(() => ({}))
  ])
    .then(([data, slugs]) => {
